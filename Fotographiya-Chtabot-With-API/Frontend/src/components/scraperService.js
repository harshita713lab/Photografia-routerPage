...
class ScraperService {
    ...
    buildContextForAI(userMessage, wantsExamples, shootType) {
        ...
        // Log the user message and detected shoot type for debugging
        ...
        // ============================================
        // 🔥 STEP 6: Add RELEVANT SCRAPED DATA snippets
        // ============================================
        const relevantScrapedSnippets = [];
        // Use a more robust way to get keywords from the user message
        const userKeywords = new Set(msgLower.split(/\s+/).filter(word => word.length > 2));

        // Add specific keywords for common queries that might be in scraped data
        const specificKeywords = [
            'album', 'handcrafted', 'handmade', 'bespoke', 'customized album', 'album design',
            'customize', 'customise', 'tailor', 'flexible', 'remove service', 'change service', 'bespoke concept',
            'goldenbox', 'golden box', 'qr service', 'qr photo', 'reelographer', 'after-sales support',
            'equipment', 'post production', 'academy', 'course', 'internship', 'portfolio', 'reviews'
        ];
        specificKeywords.forEach(kw => userKeywords.add(kw));

        let snippetCount = 0;
        const MAX_SNIPPETS = 5; // Limit the number of snippets to avoid context overflow
        const SNIPPET_LENGTH = 250; // Limit the length of each snippet

        for (const [pageKey, pageData] of Object.entries(this.scrapedData)) {
            if (!pageData.success || !pageData.content || snippetCount >= MAX_SNIPPETS) continue;

            const normalizedPageContent = normalizeText(
                (pageData.title || '') + ' ' +
            ...
            for (const keyword of userKeywords) {
                if (normalizedPageContent.includes(normalizeText(keyword))) {
            ...
            if (foundMatchInPage) {
                // Find the most relevant paragraph or take a general snippet
                const bestSnippet = pageData.paragraphs.find(p => Array.from(userKeywords).some(kw => normalizeText(p).includes(normalizeText(kw))))
                                    || pageData.content.slice(0, SNIPPET_LENGTH);
                if (bestSnippet && !relevantScrapedSnippets.some(s => s.includes(bestSnippet.trim()))) { // Avoid near-duplicates
                    relevantScrapedSnippets.push(`- From ${pageKey} page: "${bestSnippet.trim().slice(0, SNIPPET_LENGTH)}..."`);
                    snippetCount++;
                }
            }
        }

        if (relevantScrapedSnippets.length > 0) {
            contextParts.push(`🌐 **RELEVANT SCRAPED INFORMATION (FROM LIVE WEBSITE):**`);
            contextParts.push(...relevantScrapedSnippets);
            contextParts.push(``);
        } else {
            contextParts.push(`🌐 **RELEVANT SCRAPED INFORMATION (FROM LIVE WEBSITE):** No direct matches found in scraped content for your specific query. The Fotographiya team would be happy to provide more details.`);
            contextParts.push(``);
        }
